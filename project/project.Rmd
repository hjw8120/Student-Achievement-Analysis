...
We used an indpendence null hypothesis because we are comparing two variables, and used permute to generate a null distribution of the difference in median average scores between first and non-first generation students.
```{r permute}
set.seed(2019)
...
The confidence interval shows that we are 95% confident that the true difference between mathematic performance in first and non-first generation students is between -2.54 and -1.06. In other words, it is highly likely that first generation students on average score 1.06 to 2.54 points lower than their non-first generation counterparts. This difference is very significant, considering the average score for all the students is 10.70 with a standard deviation of 3.70. 
...
Since studytime and failures are both influential variables, we modeled an interaction between these two variables.
```{r interaction}
...
The rate of change in average score as studytime increases does vary between the number of failures each student has had in the past (different slopes). Some average scores based on failures is higher in some number of failures and lower in others (different intercepts).
...
The R-squared value measures the strength of the fit of a linear model. We used the adjusted R-squared value because it accounts for the additional variables in a multiple regression model. Roughly 24.5% of the variability in first generation students' average scores can be explained by gender, failures, study time, number of times going out, urban or rural address, romantic relationships, parent status, alcohol consumption, and health. Roughtly 24.0% of the variability in non-first generation students' average scores can be explained by failures, mother's job, father's job, wanting higher education, supplemental education, alcohol consumption, internet access, travel time, free time, and number of times going out.
```{r r-squared}
...
The comparative box plot demonstrates that non-first generation students obtain slightly higher average scores when they both have zero past class failures, but first generation students obtain higher scores if they have one or more past failure. We found this interesting because we would have assumed that non-first generation students would have higher scores, regardless of past-class failures. However, the bar graph below visualizes the proportion of failures in first and non-first generation students, and it demonstrates that non-first generation students have a much higher proportion of students who do not have past class failures than first generation students. Therefore, the fact that first generation students obtain higher score when they have more than one failure might just be due to the fact that the small number of non-first generation who fail one or more classes get slightly lower scores than the first generation students who, on average, fail more often. 
...
We found a significant difference (p < 0.0001) in the mathematical performance of first and non-first generation students. This rejects the null hypothesis and supports our hypothesis that there exists a difference in the mathematical performance of first and second generation students. Our confidence interval (-2.54, -1.06) suggests that the difference between their grades is huge, considering that the median grade for all students is around 10.7. This means that first generation students on average perform about 10% to even 20% lower than their non-first generation counterparts. 
